...
#include "ggml-cuda/common.cuh"  // Reuses CUDA common structures
#include "ggml-backend-impl.h"
#include <hip/hip_runtime.h>
#include <algorithm>
#include <queue>

// HIP-specific MoE cache implementation (very similar to CUDA)
struct ggml_moe_cache_hip : public ggml_moe_cache {
    // HIP-specific members
    hipStream_t transfer_stream;  // Dedicated stream for async transfers
    ...
    ggml_moe_cache_hip(
        ...
        for (int i = 0; i < num_buffers; ++i) {
            void* pinned_buffer;
            hipHostMalloc(&pinned_buffer, pinned_buffer_size, hipHostMallocDefault);
    ...
    void* acquire_pinned_buffer(size_t size) {
        ...
        if (size > pinned_buffer_size) {
            ...
            void* temp_buffer;
            hipHostMalloc(&temp_buffer, size, hipHostMallocDefault);
            return temp_buffer;
        }
        ...
        void* new_buffer;
        hipHostMalloc(&new_buffer, pinned_buffer_size, hipHostMallocDefault);
        return new_buffer;
    }
...
// HIP backend cache interface implementation
struct ggml_moe_cache_interface_hip : public ggml_moe_cache_interface {
    ...
    ggml_backend_buffer_t get_expert_async(
        ...
        ggml_moe_cache_hip* hip_cache = static_cast<ggml_moe_cache_hip*>(cache);
        hipStream_t compute_stream = static_cast<hipStream_t>(stream);
        ...
        ggml_backend_buffer_t gpu_buffer = hip_cache->load_expert_async(
            expert_id,
            expert_tensor,
            compute_stream
        );
...
// Get HIP cache interface
ggml_moe_cache_interface* ggml_moe_cache_get_interface_hip() {
    static ggml_moe_cache_interface_hip interface;
    return &interface;
}

// Update backend detection to include HIP
ggml_moe_cache_interface* ggml_moe_cache_get_interface(ggml_backend_t backend) {
    ...
    // else if (ggml_backend_is_vulkan(backend)) {
    //     return ggml_moe_cache_get_interface_vulkan();
    // }
